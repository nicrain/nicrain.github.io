...
const AppImage = () => {
        ...
        if (e && e.target && e.target.files) {
            ...
            if (selectedFiles.length > 0) {
                ...
                reader.onload = function (event) {
                    ...
                    setImage(dataURL);
                    handleFormSubmit(dataURL); // Call handleFormSubmit with the dataURL after setting the image state
                };
    ...
    const handleImageChange2 = (e) => {
        if (e && e.target && e.target.files) { // Vérifier que e et ses propriétés sont définis
            const selectedFiles = e.target.files;
            if (selectedFiles.length > 0) {
                const file = selectedFiles[0];
                const reader = new FileReader();
    ...
    const handleFormSubmit = (dataURL) => {
        const imageRef = storageRef(storage, `evenement/${id_event}/${Date.now()}.jpg`);
        return uploadString(imageRef, dataURL, 'data_url') // Utilisez dataURL plutôt que image
            .then((snapshot) => {
                return getDownloadURL(snapshot.ref); // Utilisez getDownloadURL sur le ref du snapshot
            })
            .then((downloadURL) => {
                const lieuxRef2 = ref(database, 'evenement/' + id_event + '/images');
    ...
    const handleFormSubmit3 = (dataURL) => {
        ...
        return uploadString(imageRef, dataURL, 'data_url') // Utilisez dataURL plutôt que image
            ...
            .then((downloadURL) => {
                const lieuxRef3 = ref(database, 'evenement/' + id_event + '/images/');
    ...
    useEffect(() => {
        const fetchImage = async () => {
            try {
                const imageRef = ref(database, 'evenement/' + id_event + '/images/image');
        ...
        const fetchImage2 = async () => {
            try {
                const imageRef2 = ref(database, 'evenement/' + id_event + '/images/image2');
        ...
        const fetchImage3 = async () => {
            try {
                const imageRef3 = ref(database, 'evenement/' + id_event + '/images/image3');
        ...
        <div className="App">
            ...
            {isOpenimage ? (
                ...
                <form>
                    ...
                    {/* <input id="image" type="text" onChange={(e) => setlocalimage(e.target.value)} /> */}

...
export const get_id=(uid)=>{ // depuis etab pour afficher
    const etablissementRef = ref(database, 'users/' + uid + '/etablissement/uuid');
    let uuid;
    get(etablissementRef).then((snapshot) => {
        uuid = snapshot.val();
        localStorage.setItem('uuid2', uuid);
    });
            
    return localStorage.getItem('uuid2');
}

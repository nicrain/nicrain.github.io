...
const CardLieu = ({ url, text, lieuID }) => {
  console.log(lieuID);
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const imageRef = ref(database, 'etablissement/' + lieuID + '/images/image');
    ...
    const fetchSmall_description = async () => {
      try {
        const small_despRef = ref(database, 'etablissement/' + lieuID + '/small_description');
